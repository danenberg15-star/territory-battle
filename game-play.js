                    ...
                    window.db.ref(`rooms/${window.currentRoom}/players`).once('value', pSnap => {
                        ...
                        window.db.ref(`game/${window.currentRoom}/catchAlert`).set({
                            ...
                            window.db.ref(`rooms/${window.currentRoom}/players/${window.playerId}`).update({ role: 'snitch' }).then(() => {
                                ...
                                // במצב "על זמן" — כל הגנבים נתפסו = שוטרים מנצחים
                                window.db.ref(`game/${window.currentRoom}/players`).once('value', allSnap => {
                                    const allPlayers = allSnap.val() || {};
                                    let remainingThieves = 0;
                                    Object.keys(allPlayers).forEach(id => {
                                        if (id !== victimId && allPlayers[id].role === 'thief' && !allPlayers[id].isOffline) {
                                            remainingThieves++;
                                        }
                                    });
                                    ...
                                    if (remainingThieves === 0) {
                                        ...
                                        if (window.victoryMode !== 'timer') {
                                            ...
                                            // במצב זמן — הגנב הפך ל-snitch אבל המשחק ממשיך
                                            location.reload();
                                        }
                                    }
                                });
...
function checkOfflinePlayers() {
    ...
    window.db.ref(`rooms/${window.currentRoom}/players`).once('value', snap => {
        ...
        // במצב כיבוש שטח בלבד — בדיקת ניצחון לפי שחקנים
        if (window.victoryMode !== 'timer') {
            ...
            // במצב זמן — רק אם כל הגנבים אופליין/נתפסו
            if (activeThieves === 0 && window.hasSeenThief) {
                window.db.ref(`game/${window.currentRoom}/winner`).transaction(current => current || 'cops');
            }
        }
